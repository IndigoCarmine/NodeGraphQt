...
class NodeBaseWidget(QtWidgets.QGraphicsProxyWidget):
    ...
    def set_custom_widget(self, widget, use_wheel_event=False):
        """
        ...
        self.use_wheel_event = use_wheel_event
    ...
    def customWidgetWheelEvent(self, event):
        """
        This is the default wheel event handler for the custom widget.
        You can override this method to handle the wheel event in your
        custom widget.

        Args:
            event (QtGui.QWheelEvent): wheel event.
        """
        if self.use_wheel_event:
            self.get_custom_widget().wheelEvent(event)
